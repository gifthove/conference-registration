...
server.post("/attendees/", function(req, res, next) {
  const error = validateAttendee(req.body);
  if (error) {
    res.status(400).send(error);
  } else {
    // req.body.slug = createSlug(req.body.title); // Generate a slug for new attendees.
    next();
  }
});
...
function validateAttendee(attendee) {
  if (!attendee.title) return "Title is required.";
  if (!attendee.category) return "Category is required.";
  return "";
}
